...
const staticify = require('../server.js').staticify
...
TemplateEngine.__express = function (templatePath, templateData, next) {
  let templatePaths = [templateData.settings.views, templateData.settings.vendorViews]
  let templateName
  ...
  let templates = TemplateEngine._getTemplates(templatePaths)
  let output = null
  let error = null
...
TemplateEngine._getTemplateName = function (basePath, templatePath) {
  let relativePath = Path.relative(basePath, templatePath)
  let templateName = Path.join(Path.dirname(relativePath), Path.basename(templatePath, Path.extname(templatePath)))
...
TemplateEngine._storeTemplate = function (basePath) {
  return function (templatePath) {
    let templateName = TemplateEngine._getTemplateName(basePath, templatePath)
