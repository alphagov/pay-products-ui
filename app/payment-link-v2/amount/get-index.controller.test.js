...
describe('Amount controller', () => {
  ...
  describe('when product.reference_enabled=true', () => {
    ...
    it('when reference is NOT set, then it should display the amount page', () => {
  ...
  describe('when there is already an amount in session', () => {
    const product = productFixtures.validCreateProductResponse({
      type: 'ADHOC',
      reference_enabled: false,
      price: null
    ...
    it('then it should display that amount and set the back link to the CONFIRM page', () => {
      ...
      expect(pageData.productAmount).to.equal(1000)
    })
  })
