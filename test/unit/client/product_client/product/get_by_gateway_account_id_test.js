  ...
  describe('when products are successfully found', () => {
    ...
    it('should find an existing product', () => {
      ...
      result.forEach((product, index) => {
        ...
        expect(product.payLink.href).to.exist.and.equal(`http://products-ui.url/pay/${plainResponse[index].external_id}`)
        expect(product.selfLink.href).to.exist.and.equal(`http://products.url/v1/api/products/${plainResponse[index].external_id}`)
      })
    })
  })
